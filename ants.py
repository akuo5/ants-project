...
# Name: Xinyu Liu, Angela Kuo
# Email: xinyuliu@berkeley.edu, akuo5@berkeley.edu
...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if self.ant is not None: #some Ant at place
                assert not self.ant.container or not insect.container, 'Two {0} in {1}'.format(self.ant, self) #2 BodyGuard
                #assert self.ant.container or insect.container, 'Two ants in {0}'.format(self) #2 Ant
                if self.ant.can_contain(insect) and self.ant.ant is None: #Bodyguard + Ant
                    self.ant.contain_ant(insect)
                    insect.place = self
                    self.ant.place = self
                    return #???
                elif insect.can_contain(self.ant): #Ant + Bodyguard
                    insect.contain_ant(self.ant)
                    self.ant = insect #place.ant == bodyguard
                    insect.place = self
                    self.ant.place = self
                    return #???
                assert self.ant is None, 'Two ants in {0}'.format(self) #2 Ant
            self.ant = insect #else self.ant is None
            #!!!Ant.place, BodyguardAnt.place == self. place.ant only == BodyguardAnt   
        else:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if insect.container and insect.ant != None: #BodyguardAnt removed, containing Ant takes Place
                self.ant = insect.ant
                self.ant.place = self
                insect.place = None
                return
            elif type(insect) == QueenAnt and insect.true_queen == True: #Q9: true QueenAnt cannot be removed
                return
            self.ant = None #regular Ant removed
        insect.place = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current.entrance is not None:
            if current.bees != [] and (count >= self.min_range and count <= self.max_range):
...
class AntColony:
    ...
    def simulate(self):
        ...
        while len(self.queen.bees) == 0 and len(self.bees) > 0: 
            self.hive.strategy(self)    # Bees invad
...
def make_test_assault_plan():
    return AssaultPlan().add_wave(2, 1)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        "*** YOUR CODE HERE ***"
        Place.add_insect(self, insect) #???Water.add_insect(self, insect)
...
"*** YOUR CODE HERE ***"
class WallAnt(Ant):
    """WallANT is an Ant that has a large armor and does nothing each turn"""
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        if self.ant != None: #!!!
            self.ant.action(colony) #(self, colony)???
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        "*** YOUR CODE HERE ***"
        self.colony_queen = colony_queen #Place
        self.ant_queen = ant_queen #Place

    @property
    def bees(self):
        "*** YOUR CODE HERE ***"
        return self.colony_queen.bees + self.ant_queen.bees #a list of Bees
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        if QueenAnt.num_queen > 1: #imposter QueenAnt
            self.true_queen = False
        else:
            self.true_queen = True
            QueenAnt.queen_ants.append(self)
            self.queen_first_action = True
            #print(QueenAnt.queen_ants)
    ...
    def action(self, colony):
        ...
        if self.queen_first_action:
            colony.queen = QueenPlace(colony.queen, self.place)
            self.queen_first_action = False
        ScubaThrower.action(self, colony)

        def double_damage(place):
            if place.ant.container:
                place.ant.damage *= 2
                if place.ant.ant is not None and place.ant.ant not in self.queen_ants: #Bodyguard contains Ant at Place
                    place.ant.ant.damage *= 2
            elif not place.ant.container:
                place.ant.damage *= 2

        place = self.place

        while place.entrance is not None:
            if place.ant is not None and place.ant not in self.queen_ants:
                double_damage(place)
                self.queen_ants.append(place.ant)
                print(QueenAnt.queen_ants)
